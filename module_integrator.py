...
from daceypy import DA, array, RK, integrator, ADS


def RK78(Y0: array, X0: float, X1: float, f: Callable[[array, float], array]):
    """
    Runge-Kuta (7,8) solver
    :param Y0: initial state (x0)
    :param X0: initial epoch (t0)
    :param X1: final epoch (tf)
    :param f: dynamics (derivatives)
    :return: numerical integration solution at the final epoch
    """
    Y0 = Y0.copy()
    ...
    while X != X1:
        ...
        if abs(H) > abs(HH1):
            ...
            # print(Y0.cons())
            raise ValueError("MINIMUM STEPSIZE REACHED IN RK.")
...
def base_propagation(domain_0: ADS, t0: float, tf: float, f: Callable[[array, float], array]) -> ADS:
    """
...
def advanced_propagation(domain_0: ADS, t0: float, tf: float, f: Callable[[array, float], array]) -> ADS:
    """
