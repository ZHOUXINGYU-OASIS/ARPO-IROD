...
def RIOD_convex_constrained_optimization(
        guess: np.array,
        STMs: np.array,
        Maps: list,
    ...
    """This code is the convex optimization with non-zero constraint to avoid undesired solution"""
    ...
    with mosek.Task() as task:
        ...
        """Add the bound constraints on designed variables"""
        ...
        prob_a = prob_a.T
        prob_a = sparse.lil_matrix(prob_a)
        asub = prob_a.rows
        aval = prob_a.data
        for k in range(DIMx):
            task.putacol(k,  # Variable (column) index.
                         asub[k],  # Row index of non-zeros in column j.
                         aval[k])  # Non-zero Values of column j.
...
def RIOD_convex_constrained_optimization_weighted(
    ...
    num_observations = len(los_vectors)
    DIM = 6
    DIMx = 6  # for weighted problem, using only position variables for zero-avoidance constraint
    DIMz = 3
    """Add weights"""
